            ...
            <div class="col-12">
                ...
                <div class="card">
                    ...
                    <div class="card-header">
                        <h3 class="card-title"><?=$brand_master_data->brand_name?></h3>
                        <div class="float-right">
                            <?php 
								if($user_access->add_module==1 && false)	{
								?>
								<a href="<?=MAINSITE_Admin.$user_access->class_name?>/brand-edit"> 
							...
							?>
							<a href="<?=MAINSITE_Admin.$user_access->class_name?>/brand-edit/<?=$brand_master_data->brand_id?>"> 
                    ...
                    <div class="card-body">
                            ...
                            <table id="" class="table table-bordered table-hover myviewtable responsiveTableNewDesign">
								...
								<tr>
										...
										<?=$brand_master_data->brand_id?></td>
										<td>
										<strong class="full">Brand</strong>
										<?=$brand_master_data->brand_name?></td>
										<td>
										<strong class="full">Added On</strong>
										<?=date("d-m-Y h:i:s A" , strtotime($brand_master_data->added_on))?></td>
										<td >
										<strong class="full">Added By</strong>
										<?=$brand_master_data->added_by_name?></td>
										<td  >
										<strong class="full">Updated On</strong>
										<? if(!empty($brand_master_data->updated_on)){echo date("d-m-Y h:i:s A" , strtotime($brand_master_data->updated_on));}else{echo "-";}?></td>
										<td>
										<strong class="full">Updated By</strong>
										<? if(!empty($brand_master_data->updated_by_name)){echo $brand_master_data->updated_by_name;}else{echo "-";}?></td>
										<td >
										<strong class="full">Status</strong>
										<? if($brand_master_data->status==1){ ?> Active <i class="fas fa-check btn-success btn-sm "></i>
