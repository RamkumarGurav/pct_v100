...
$(document).ready(function() {
   ...
   //dataType : "json", 
   data : {"id" : '<? echo $id; ?>' , "withPosition" : 1  , 'sortByPosition':1 , "<?=$csrf['name']?>":"<?=$csrf['hash']?>"},
	 ...
	 $("#table-2").tableDnD({
	    onDragClass: "myDragClass",
			onDrop: function(table, row) {
				...
				$.ajax({
			   type: "POST",
			   url: '<?=MAINSITE_Admin.'gallery/Gallery-Module/GetCompleteGalleryListNewPos'?>',
			   //dataType : "json", 
			   data : {"id" : '<? echo $id; ?>' , 'podId':podId , "<?=$csrf['name']?>":"<?=$csrf['hash']?>"},
			   success : function(result){
				  // alert(result);
					$('#galleryList').html(result);
