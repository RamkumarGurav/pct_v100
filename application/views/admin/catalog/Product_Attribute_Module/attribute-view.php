            ...
            <div class="col-12">
                ...
                <div class="card">
                    ...
                    <div class="card-header">
                        <h3 class="card-title"><?=$product_attribute_value_detail->name?></h3>
                        <div class="float-right">
							...
							?>
							<a href="<?=MAINSITE_Admin.$user_access->class_name?>/attribute-edit/<?=$product_attribute_value_detail->product_attribute_value_id?>"> 
                    ...
                    <div class="card-body">
                            ...
                            <div class="divTable">
                                ...
                                <div class="TableRow">
                                    ...
                                    <?=date("d-m-Y h:i:s A" , strtotime($product_attribute_value_detail->added_on))?>
                                    </div>
                                    <div class="table_col">
                                    <label class="label_content_br">Added By <span class="colen">:</span></label>
                                    <?=$product_attribute_value_detail->added_by_name?>
                                    </div>
                                    <div class="table_col">
                                    <label class="label_content_br">Updated On <span class="colen">:</span></label>
                                   <? if(!empty($product_attribute_value_detail->updated_on)){echo date("d-m-Y h:i:s A" , strtotime($product_attribute_value_detail->updated_on));}else{echo "-";}?>
                                    </div>
                                    <div class="table_col">
                                    <label class="label_content_br">Updated By <span class="colen">:</span></label>
                                    <? if(!empty($product_attribute_value_detail->updated_by_name)){echo $product_attribute_value_detail->updated_by_name;}else{echo "-";}?>
                                    </div>
                                    <div class="table_col">
                                    <label class="label_content_br">Status <span class="colen">:</span></label>
                                   <? if($product_attribute_value_detail->status==1){ ?> Active <i class="fas fa-check btn-success btn-sm "></i>
                            ...
                            <table id="" class="table table-bordered table-hover myviewtable" style="display:none;">                                
                                <tbody>                                    
									<tr>
										...
										<?=$product_attribute_value_detail->city_id?></td>
										<td><strong class="full">Country</strong>
										<?=$product_attribute_value_detail->country_name?></td>
										<td><strong class="full">State</strong>
										<?=$product_attribute_value_detail->state_name?></td>
										<td><strong class="full">City</strong>
										<?=$product_attribute_value_detail->city_name?></td>
										<td><strong class="full">City Code</strong>
										<?=$product_attribute_value_detail->city_code?></td>
										<td><strong class="full">is Display</strong>
										<? if($product_attribute_value_detail->is_display==1){ ?> Yes <i class="fas fa-check btn-success btn-sm "></i>
                                                <?}else{ ?> No <i class="fas fa-ban btn-danger btn-sm "></i>
                                                <? }?></td>
									</tr>									
									<tr>
										<td><strong class="full">Added On</strong>
										<?=date("d-m-Y h:i:s A" , strtotime($product_attribute_value_detail->added_on))?></td>
										<td><strong class="full">Added By</strong>
										<?=$product_attribute_value_detail->added_by_name?></td>
										<td><strong class="full">Updated On</strong>
										<? if(!empty($product_attribute_value_detail->updated_on)){echo date("d-m-Y h:i:s A" , strtotime($product_attribute_value_detail->updated_on));}else{echo "-";}?></td>
										<td><strong class="full">Updated By</strong>
										<? if(!empty($product_attribute_value_detail->updated_by_name)){echo $product_attribute_value_detail->updated_by_name;}else{echo "-";}?></td>
