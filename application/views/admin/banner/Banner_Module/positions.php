...
$(document).ready(function() {
   ...
   //dataType : "json",
   data : {"banner_id" : '<? echo $banner_id; ?>' , "withPosition" : 1  , 'sortByPosition':1 , "<?=$csrf['name']?>":"<?=$csrf['hash']?>"},
	 ...
	 $("#table-2").tableDnD({
	    onDragClass: "myDragClass",
			onDrop: function(table, row) {
				...
				$.ajax({
			   type: "POST",
			   url: '<?=MAINSITE_Admin.'banner/Banner-Module/GetCompleteBannerListNewPos'?>',
			   //dataType : "json",
			   data : {"banner_id" : '<? echo $banner_id; ?>' , 'podId':podId , "<?=$csrf['name']?>":"<?=$csrf['hash']?>"},
			   success : function(result){
				  // alert(result);
					$('#bannerList').html(result);
